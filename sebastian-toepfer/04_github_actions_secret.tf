...
resource "github_actions_secret" "domain-driven-desgin_mvn-user" {
  repository      = github_repository.domain-driven-desgin.name
  secret_name     = "mvn_username"
  plaintext_value = var.mvn_repo_user
}

resource "github_actions_secret" "domain-driven-desgin_mvn-pwd" {
  repository      = github_repository.domain-driven-desgin.name
  secret_name     = "mvn_password"
  plaintext_value = var.mvn_repo_pwd
}

resource "github_actions_secret" "domain-driven-desgin_gpg-pwd" {
  repository      = github_repository.domain-driven-desgin.name
  secret_name     = "GPG_PASSPHRASE"
  plaintext_value = var.gpg_pwd
}

resource "github_actions_secret" "domain-driven-desgin_gpg-key" {
  repository      = github_repository.domain-driven-desgin.name
...
resource "github_actions_secret" "pdfbox-fluent-layout_mvn-user" {
  repository      = github_repository.pdfbox-fluent-layout.name
  secret_name     = "mvn_username"
  plaintext_value = var.mvn_repo_user
}

resource "github_actions_secret" "pdfbox-fluent-layout_mvn-pwd" {
  repository      = github_repository.pdfbox-fluent-layout.name
  secret_name     = "mvn_password"
  plaintext_value = var.mvn_repo_pwd
}

resource "github_actions_secret" "pdfbox-fluent-layout_gpg-pwd" {
  repository      = github_repository.pdfbox-fluent-layout.name
  secret_name     = "GPG_PASSPHRASE"
  plaintext_value = var.gpg_pwd
}

resource "github_actions_secret" "pdfbox-fluent-layout_gpg-key" {
  repository      = github_repository.pdfbox-fluent-layout.name
  secret_name     = "GPG_PRIVATE_KEY"
  plaintext_value = var.gpg_key
}
...
resource "github_actions_secret" "json-rpc_mvn-user" {
  repository      = github_repository.json-rpc.name
  secret_name     = "mvn_username"
  plaintext_value = var.mvn_repo_user
}

resource "github_actions_secret" "json-rpc_mvn-pwd" {
  repository      = github_repository.json-rpc.name
  secret_name     = "mvn_password"
  plaintext_value = var.mvn_repo_pwd
}

resource "github_actions_secret" "json-rpc_gpg-pwd" {
  repository      = github_repository.json-rpc.name
  secret_name     = "GPG_PASSPHRASE"
  plaintext_value = var.gpg_pwd
}

resource "github_actions_secret" "json-rpc_gpg-key" {
  repository      = github_repository.json-rpc.name
  secret_name     = "GPG_PRIVATE_KEY"
  plaintext_value = var.gpg_key
}
